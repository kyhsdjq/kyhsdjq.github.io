...
categories: [Java]
tags: [subject, computer science, java, concurrency]     # TAG names should always be lowercase
author: kyhsdjq
...
ZGC 中每个区域都有自己的**转移表**，于是该区域的锁存放在转移表中。以下是在转移中确保并发安全的两个手段：
- 引用计数：确认区域是否正被使用
...
这个函数通过与 _ref_count 的原子交互来获取和更新状态。
...
它总是**乐观**地先复制对象，然后通过 forwarding_insert 尝试将新地址插入转移表，如果发现别的线程在之前插入过了，再进行回滚。
